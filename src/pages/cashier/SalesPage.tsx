...
export function SalesPage() {
  ...
  const filtered = sales.filter((s) => {
    const matchesSearch = !search || s.receipt_number.toLowerCase().includes(search.toLowerCase()) || (s.mpesa_reference ?? '').toLowerCase().includes(search.toLowerCase());
    const matchesStatus = statusFilter === 'all' || s.payment_status === statusFilter;
    return matchesSearch && matchesStatus;
  });

  if (loading) {
    return <div className="p-8 text-center text-slate-400"><Loader2 className="animate-spin inline mr-2" size={20} />Loading sales...</div>;
  }
  ...
  return (
    <div className="p-6 lg:p-8 max-w-7xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-slate-500 mt-1">View your recent sales transactions.</p>
      </div>
      ...
      {filtered.length === 0 ? (
        <EmptyState icon={<Receipt size={48} />} title="No sales found" description="Sales will appear here once you start making transactions." />
      ) : (
                ...
                {filtered.map((s) => (
                  <tr key={s.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3">
                      <span className={
                        s.payment_status === 'success' ? 'badge-success' :
                        s.payment_status === 'pending' ? 'badge-warning' :
                        'badge-danger'
